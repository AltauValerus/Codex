...
def ensure_non_empty_split(split_name: str, X_part: pd.DataFrame, y_part: np.ndarray) -> None:
    if len(X_part) == 0 or len(y_part) == 0:
        raise SystemExit(
            f"{split_name} split is empty after filtering. "
            "Adjust --train-end/--val-end to ensure train/val/test all contain rows."
        )


def ensure_binary_labels(split_name: str, y_part: np.ndarray) -> None:
    uniques = np.unique(y_part)
    if len(uniques) < 2:
        raise SystemExit(
            f"{split_name} split has only one class: {uniques.tolist()}. "
            "Metrics like AUC require both classes."
        )
...
def main() -> None:
    ...
    ensure_non_empty_split("train", X_train, y_train)
    ensure_non_empty_split("val", X_val, y_val)
    ensure_non_empty_split("test", X_test, y_test)

    ensure_binary_labels("train", y_train)
    ensure_binary_labels("val", y_val)
    ensure_binary_labels("test", y_test)
